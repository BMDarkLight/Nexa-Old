...
def _read_pdf_logic(settings: Dict[str, Any], query: str) -> str:
    ...
    if not document_id:
        return "Error: Connector is misconfigured. 'document_id' is missing from settings."

    try:
        source_document = knowledge_db.find_one({"_id": ObjectId(document_id)})
    except Exception as e:
        return f"Error: Invalid 'document_id' format or database error: {e}"

    if not source_document or "chunks" not in source_document:
        return f"Error: No document or text chunks found for the provided ID: {document_id}."
